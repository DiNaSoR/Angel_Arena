...
function 	Spawn( entityKeyValues )
				Pudgespeech = true
				print("Starting AI for "..thisEntity:GetUnitName().." "..PUDGE_AI_VERSION)
...
function dialogpudge()
	...
	if speechSlot < 4 and Pudgespeech == true then
		Pudgespeech = false
		thisEntity:AddSpeechBubble(speechSlot, PUDGE_DIALOG[RandomInt(1,7)], time, 0, -50)
		EmitGlobalSound(RUNNING_SOUNDBOX[RandomInt(1,7)])
		disableSpeech(thisEntity, time, speechSlot)
		Timers:CreateTimer(5, function()
      	Pudgespeech = true
    	end)
	else
		--print ("On speech are false")
	end
end
